...
ServerEvents.ballHit.Event:Connect(function(player, cameraDirection)
	...
	print("BallServer: Hit event received from", player.Name, "GameId:", gameId)
	if not gameId or not ActiveBalls[gameId] then 
		warn("BallServer: GameId not found or no active ball for", gameId)
		return 
	end
